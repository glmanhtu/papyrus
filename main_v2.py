...
class FakeDataLoader:
    ...
    def __init__(self, datasets, batch_size, m, numb_workers, pin_memory, repeat, repeat_same_class):
        ...
        self.sampler = FakeDataLoader.FakeSampler()

    def __len__(self):
        return min([len(x) for x in self.dataloaders])
...
class TripletDistanceLoss(torch.nn.Module):
    ...
    def forward(self, features, targets):
        geshame_samples = features.size(0) // 2
        samples = features[:geshame_samples]
        targets = targets[:geshame_samples]
        michigan_features = features[geshame_samples:]
...
class GeshaemTrainer(Trainer):
    ...
    def get_dataloader(self, mode, dataset, data_conf):
        ...
        if mode == 'train':
            dataloader = FakeDataLoader(dataset, batch_size=data_conf.batch_size, m=data_conf.m_per_class,
                                        numb_workers=data_conf.num_workers, pin_memory=True,
                                        repeat=data_conf.m_per_class, repeat_same_class=False)
        else:
    ...
    def validate_one_epoch(self, dataloader):
        ...
        self.logger.info(f'Average: \t mAP {m_ap:.4f}\t top1 {top1:.3f}\t'
